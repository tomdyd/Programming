...
const prepareDOMElements = () => {
  ...
  getRawMaterials = document.querySelector(".getRawMaterials");
...
const prepareDOMEvents = () => {
  ...
  getRawMaterials.addEventListener("click", () => addHeaders(dataList), {
    once: true,
  });
  getRawMaterials.addEventListener(
    "click",
    () => {
      getRawMaterialList(dataList);
    },
    { once: true }
  );
  formEl.addEventListener("submit", addNewRawMaterial);
...
const getRawMaterialList = (list) => {
  fetch(URL + `rawMaterials`, {
    method: "GET",
  })
    .then((res) => {
      return res.json();
...
const updateData = (index) => {
  ...
  listItems.forEach((item) => {
    if (editInput.value === item.textContent) {
      editData = item.closest(".dataList").querySelectorAll("li");
      inputs = editValves.querySelectorAll("input");

      for (let i = 0; i < inputs.length; i++) {
        editData[i].textContent = inputs[i].value;
      }
    }
  });
